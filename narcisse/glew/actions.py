...
def build():
    if get.buildTYPE() == "emul32":
        ...
        autotools.make('all CC="%s -m32" CXXFLAGS="%s"' % (get.CC(), get.CXXFLAGS()))
        return
    else:
        autotools.make('all CC=%s CXXFLAGS="%s"' % (get.CC(), get.CXXFLAGS()))
        return

def install():
    if get.buildTYPE() == "emul32":
        autotools.rawInstall("install.all \
                              GLEW_DEST=%s/usr/ \
                              INCDIR=%s/emul32 \
                              BINDIR=%s/emul32 \
        ...
        pisitools.remove("/usr/lib32/libGLEWmx.a")
        return
    ...
    pisitools.remove("/usr/lib/libGLEWmx.a")
