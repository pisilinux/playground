...
def setup():
    ...
    shelltools.system("../configure --prefix=/opt/firefox-beta --libdir=/opt/firefox-beta/lib --disable-strip --disable-install-strip")
...
def install():
    ...
    pisitools.insinto("/opt/firefox-beta/lib/firefox/browser/extensions", "./langpack-ff/*")

    # Create profile dir, we'll copy bookmarks.html in post-install script
    pisitools.dodir("/opt/firefox-beta/lib/firefox/browser/defaults/profile")

    # Install branding icon
    pisitools.insinto("/usr/share/pixmaps", "browser/branding/official/default256.png", "firefox-beta.png")
    pisitools.dosym("/opt/firefox-beta/lib/firefox/firefox", "/usr/bin/firefox-beta")
