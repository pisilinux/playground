...
def postInstall(fromVersion, fromRelease, toVersion, toRelease):
    ...
    os.system("/usr/sbin/alternatives \
               --install /usr/lib32/libEGL.so.1.2.0 libEGL-32bit /usr/lib32/mesa/libEGL.so.1.0.0 80 \
               --slave /usr/lib32/xorg/modules/volatile xorg-modules-volatile-32bit /var/empty")

    if not os.path.lexists("/usr/lib32/libGL.so.1"):
        os.symlink("libGL.so.1.2.0", "/usr/lib32/libGL.so.1")
    
    if not os.path.lexists("/usr/lib32/libEGL.so.1.1.0"):
		os.symlink("libEGL.so.1.0.0", "/usr/lib32/libEGL.so.1.1.0")
		
    if not os.path.lexists("/usr/lib32/libEGL.so.1"):
		os.symlink("libEGL.so.1.0.0", "/usr/lib32/libEGL.so.1")	
	
    if not os.path.lexists("/usr/lib32/libEGL.so"):
		os.symlink("libEGL.so.1.0.0", "/usr/lib32/libEGL.so")	
