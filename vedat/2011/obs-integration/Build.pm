...
sub import {
  for (@_) {
    ...
    $do_pisi = 1 if $_ eq ':pisi';
  }
  $do_rpm = $do_deb = $do_kiwi = $do_pisi = 1 if !$do_rpm && !$do_deb && !$do_kiwi && !$do_pisi;
  if ($do_deb) {
    require Build::Deb;
  }
  if ($do_kiwi) {
...
sub parse {
  ...
  return Build::Pisi::parse($fn) if $do_pisi && $fn =~ /^pspec.xml$/;
  return undef;
}
...
sub query {
  ...
  return Build::Pisi::query($handle, %opts) if $do_pisi && $binname =~ /\.pisi$/;
  return undef;
}
...
sub queryhdrmd5 {
  ...
  return Build::Pisi::queryhdrmd5(@_) if $do_pisi && $binname =~ /\.pisi$/;
  return undef;
}
