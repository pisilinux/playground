...
Archives = "jdk-%s-linux-%s-demos.tar.gz" % (Ver, Arch)
Url = "http://download.oracle.com/otn-pub/java/jdk/%s/%s" % (Dir, Archive)
Urls = "http://download.oracle.com/otn-pub/java/jdk/%s-demos/%s" % (Dir, Archives)
def setup():
    ...
    if not shelltools.isFile(Archive):
        shelltools.system('/usr/bin/curl -fLC - --retry 3 --retry-delay 3 -o %s %s  --header "Cookie:oraclelicensejdk-%s-oth-JPR=accept-securebackup-cookie;gpw_e24=http://edelivery.oracle.com"' % (Archives, Urls, Ver))
    hash = run_batch("cat %s.dog" % Arch)[1].split()[0]
    if not check_file_hash(Archives, hash): raise Error("Wrong sha1sum.")
    shelltools.system("tar -xzf %s" % Archives)
